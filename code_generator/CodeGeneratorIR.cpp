...
#include "CodeGeneratorIR.h"
#include "../util/panic.h"
#include <utility>

void CodeGeneratorIR::visit(ExpressionNode &node)
{
...
void CodeGeneratorIR::visit(BinaryExpressionNode &expr)
{
...
void CodeGeneratorIR::visit(UnaryExpressionNode &expr)
...
void CodeGeneratorIR::visit(IdentSelectorExpressionNode &node)
{
...
void CodeGeneratorIR::LoadIdentSelector(IdentNode &ident, SelectorNode *selector, bool return_pointer)
{
...
void CodeGeneratorIR::visit(IdentNode &ident)
{
...
void CodeGeneratorIR::LoadIdent(IdentNode &ident, bool return_pointer)
{
    assert(!ident.get_type_embedding().has_value());
    std::string name = ident.get_value();
    auto p = variables_.lookup(name);
...
void CodeGeneratorIR::visit(IntNode &val)
{
...
void CodeGeneratorIR::visit(SelectorNode &)
{
    panic("unreachable this should be handled in IdentSelectorExpressionNode");
}

void CodeGeneratorIR::visit(DeclarationsNode &node)
{
    create_declarations(node, false);
}

void CodeGeneratorIR::create_declarations(DeclarationsNode &node, bool is_global)
{
...
void CodeGeneratorIR::visit(TypeNode &node)
{
    switch (node.getNodeType())
    {
    case NodeType::ident:
...
void CodeGeneratorIR::visit(ArrayTypeNode &node)
{
...
void CodeGeneratorIR::visit(RecordTypeNode &node)
{
...
void CodeGeneratorIR::visit(ProcedureDeclarationNode &node)
{
...
void CodeGeneratorIR::visit(StatementNode &node)
{
    switch (node.getNodeType())
    {
...
void CodeGeneratorIR::visit(AssignmentNode &node)
{
...
void CodeGeneratorIR::visit(IfStatementNode &node)
{
...
void CodeGeneratorIR::visit(ProcedureCallNode &node)
{
    // Get ProcedureName
...
void CodeGeneratorIR::visit(RepeatStatementNode &node)
{
    // Create Basic Blocks
...
void CodeGeneratorIR::visit(StatementSequenceNode &node)
{
...
void CodeGeneratorIR::visit(WhileStatementNode &node)
{
...
void CodeGeneratorIR::visit(ModuleNode &node)
{

    variables_.beginScope();
    type_table_.beginScope();

    type_table_.insert("INTEGER", TypeInfoClass(INTEGER_TAG, {IRType::getInt64Ty()}, {}));
    type_table_.insert("BOOLEAN", TypeInfoClass(BOOLEAN_TAG, {IRType::getInt1Ty()}, {}));
...
void CodeGeneratorIR::generate_code(ModuleNode &node)
{
